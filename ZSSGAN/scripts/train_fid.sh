...
# source_class=Wild
# img_root=../img/Dataset/train_FID/Wild
...
source_class=AF
img_root=../img/Dataset/train_FID/AF

for img_folder in $(ls $img_root)
do
    for img in $(ls $img_root/$img_folder)
    do
        CUDA_VISIBLE_DEVICES=$cuda_id python train.py  \
                        ...
                        --style_img_dir "$img_root/$img_folder/$img" 
    done
done